...
class Explore : public SingleInputNode {
 public:
  ...
  // Get the constant skip value
  int64_t skip(QueryContext* qctx = nullptr) const;
  ...
  // Get the skip value in runtime
  int64_t skip(QueryExpressionContext& ctx) const {
    return DCHECK_NOTNULL(skip_)->eval(ctx).getInt();
  }
  ...
  void setSkip(int64_t skip) {
    skip_ = ConstantExpression::make(qctx_->objPool(), skip);
  }
